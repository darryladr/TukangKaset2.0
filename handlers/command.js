...
console.log("Welcome to handler service.");
module.exports = (client) => {
	...
	client.on("ready", () => {
		...
		client.user.setActivity(`${config.prefix}help | @darryladr`, {
			type: "LISTENING",
		});
		setInterval(() => {
	...
	client.distube
		...
		.on("addSong", (message, queue, song) => {
			try {
				return functions.embedbuilder(
					...
					} song(s) - \`${(Math.floor(((queue.duration - song.duration) / 60) * 100) / 100)
						.toString()
		...
		.on("playList", (message, queue, playlist, song) => {
			...
			functions.embedbuilder(
				...
				} songs\` \n\nRequested by: ${queue.songs[0].user ? queue.songs[0].user : "error"}`,
