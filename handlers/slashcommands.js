...
module.exports = (client) => {
	client.on("ready", () => {
		...
		client.api.applications(client.user.id).commands.post({
			data: {
				name: "help",
				description: "Shows you information for every command",
			},
		});
		client.api.applications(client.user.id).commands.post({
			data: {
				name: "info",
				description: "See some information about Tukang Kaset",
			},
		});
		client.api.applications(client.user.id).commands.post({
			data: {
		...
		client.ws.on("INTERACTION_CREATE", async (interaction) => {
			...
			const inviteembed = new Discord.MessageEmbed()
				...
				.setDescription(
					`[\`Click here\`](https://discord.com/api/oauth2/authorize?client_id=780669724451143701&permissions=3155968&scope=bot) | [\`GitHub\`](https://github.com/darryladr) | :heart: Thank You!`
				)
				.setFooter(
					client.user.username + " | Syntax:  <>...required    []...optional",
			...
			const infoembed = new Discord.MessageEmbed()
				.setAuthor(
					`Information about ${client.user.username}`,
					client.user.displayAvatarURL()
				)
				.setColor(config.colors.yes)
				.addFields(
					...
					{
						name: "Developer",
						value: `**\`@darryladr\` <@188510282958503936>**`,
						inline: true,
					}
				)
				.addField(
			...
			const helpembed = new Discord.MessageEmbed()
                ...
                *more coming soon*
				...
				.addField(
					"**__Music - Supported sources__**",
					`
                >>> \`Youtube\`, \`Spotify\`, \`Soundcloud\`, [\`More\`](https://links.musicium.eu), ...
                `
				)
				.setFooter(
					`To see command descriptions and usage type ${config.prefix}help [Command Name]`,
					client.user.displayAvatarURL()
				);
			...
			const info = client.categories
				.map(
					(cat) => stripIndents`**__${cat[0].toUpperCase() + cat.slice(1)}__** \n> ${commands(cat)}`
				)
				.reduce((string, category) => string + "\n\n" + category);
			helpembed.setDescription(
				"*use the Prefix infront of EACH command, to use it correctly!*\n" + info
			);
			if (command == "help") {
