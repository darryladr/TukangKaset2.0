...
function check_if_dj(message, member) {
	...
	if (!isdj && !member.hasPermission("ADMINISTRATOR")) return string;
	...
	try {
		...
		for (let i = 1; i < queue.songs.length; i += 10) {
			...
			const info = current
				.map(
					(track) =>
						`**${j++} -** [\`${track.name}\`](${track.url}) - \`${track.formattedDuration}\``
				)
				.join("\n");
	...
	try {
		...
		let embed1 = new Discord.MessageEmbed()
			...
			.addField(
				...
				`>>> ${
					queue.songs[1]
						? `\`${queue.songs[1].name}\` - \`${song.formattedDuration}\``
						: "`Nothing`"
				}`,
				true
			)
			...
			.addField(
				"♾ Loop:",
				`>>> ${queue.repeatMode ? (queue.repeatMode === 2 ? "✅ Queue" : "✅ Song") : "❌"}`,
				true
			)
			.addField("↪️ Autoplay:", `>>> ${queue.autoplay ? "✅" : "❌"}`, true)
			.addField("❔ Filter:", `>>> \`${queue.filter || "❌"}\``, true)
			.setAuthor(`${client.user.username}`, "https://i.imgur.com/joyh0Ti.gif")
			.setFooter(
				message.author.tag,
				message.member.user.displayAvatarURL({
					dynamic: true,
		...
		collector.on("collect", async (reaction, user) => {
			...
			if (check_if_dj(reaction.message, member))
				return embedbuilder(
					...
					config.colors.no,
					"DJ-ROLE",
					`❌ You don\'t have permission for this Command! You need to have: ${check_if_dj(
						message
					)}`
				);
...
function curembed(client, message) {
	try {
		...
		embed = new Discord.MessageEmbed()
			...
			.addField("💡 Requested by:", `>>> ${song.user}`, true)
			.addField("⏱ Duration:", `>>> \`${song.formattedDuration}\``, true)
			.addField("📬 Next Up:", `>>> \`${queue.songs[1].name} - ${queue.formattedDuration}\``, true)
			.addField("🔊 Volume:", `>>> \`${queue.volume}%\``, true)
			.addField(
				"♾ Loop:",
				`>>> ${queue.repeatMode ? (queue.repeatMode === 2 ? "✅ Queue" : "✅ Song") : "❌"}`,
