...
module.exports = {
	...
	usage: `${config.prefix}adddj @role`,
	run: async (client, message, args) => {
		...
		try {
			...
			return functions.embedbuilder(
				...
				config.colors.no,
				`ERROR`,
				`It seems that the Role does not exist in this Server!`
			);
		}
		...
		if (!role)
			return functions.embedbuilder(
				...
				config.colors.no,
				`ERROR`,
				`Please add a Role via ping, @role!`
			);
		if (client.settings.get(message.guild.id, `djroles`).includes(role.id))
			return functions.embedbuilder(
				...
				config.colors.no,
				`ERROR`,
				`This Role is alerady in the List!`
			);
		...
		return functions.embedbuilder(
			...
			config.colors.yes,
			"DJ-ROLE",
			`✅ Successfully set the DJ ROLE to ${role}
    All Dj Roles:
    > ${leftb}`
		);
	},
