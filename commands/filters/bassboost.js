    ...
    if (args[0]) {
      ...
      switch (Math.floor(Number(args[0]))) {
        case 1:
          ...
          await functions.embedbuilder(client, 5000, message, config.colors.yes, "Adding filter!", `Bassboost with: \`${Math.floor(Number(args[0]))}db Gain\``)
          break;
        ...
        default:
          await functions.embedbuilder(client, 5000, message, config.colors.no, "ERROR", `Bassboost with: \`${Math.floor(Number(args[0]))}db Gain\` DOES NOT WORK!`)
          break;
      }
