...
module.exports = {
	...
	aliases: ["p"],
	cooldown: 5,
	usage: `${config.prefix}play <Song URL | Song Name>`,
	...
	run: async (client, message, args) => {
		...
		if (!args[0])
			return functions.embedbuilder(
				...
				message,
				config.colors.no,
				"`" + message.author.tag + "`" + " Please add something you wanna search to"
			);
		...
		if (!message.guild.me.permissionsIn(message.member.voice.channel).has("CONNECT"))
			return functions.embedbuilder(
				...
				message,
				config.colors.no,
				"`" + message.author.tag + "`" + " I am not allowed to `join` your Channel"
			);
		...
		if (!message.guild.me.permissionsIn(message.member.voice.channel).has("SPEAK"))
			return functions.embedbuilder(
				...
				message,
				config.colors.no,
				"`" + message.author.tag + "`" + " I am not allowed to `speak` your Channel"
			);
		...
		// 		config.colors.yes,
		// 		"🎶  Fetching the songs!",
		// 		"This will take me around about: " + items.len / 2 + " seconds"
		// 	);
		// 	for (let i = 0; i < 25; i++) {
		// 		let result = await client.distube.play(message, items[i].name);
		// 		songsarray.push(result[i].uri);
		// 	}
		// 	client.distube.playCustomPlaylist(message, songsarray, {
