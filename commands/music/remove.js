...
module.exports = {
    ...
    category: "music",
    aliases: ["rt"],
    usage: `${config.prefix}remove <Queue Number>`,
    ...
    run: async (client, message, args) => {
        ...
        //if no args return
        if (!args[0]) return functions.embedbuilder(client, 5000, message, config.colors.no, "Please add the Track Position you want to Remove");
        
        //if args too big
        if (isNaN(args[0]) || Number(args[0]) >= queue.songs.length) return functions.embedbuilder(client, 5000, message, config.colors.no, "Your Song Position is out of RANGE! Max: " + queue.songs.length);
        ...
        //Send info message
        functions.embedbuilder(client, 5000, message, config.colors.yes, "Success fully removed your Track", `[${track.name}](${track.url})`)
    }
};