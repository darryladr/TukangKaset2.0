...
module.exports = {
    ...
    category: "music",
    aliases: ["repeat"],
    usage: `${config.prefix}loop <0/1/2> |`,
    ...
    run: async (client, message, args) => {
        ...
        if (0 <= loopis && loopis <= 2) {
            ...
            await functions.embedbuilder(client, 5000, message, config.colors.yes, "Repeat mode set to:", `${args[0].replace("0", "OFF").replace("1", "Repeat song").replace("2", "Repeat Queue")}`)
            return;
        } else {
            return functions.embedbuilder(client, 5000, message, config.colors.no, "ERROR", `Please use a number between **0** and **2**   |   *(0: disabled, 1: Repeat a song, 2: Repeat all the queue)*`)
        }
    }
};
